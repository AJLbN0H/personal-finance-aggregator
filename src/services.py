...
def search_individual_transfers(transactions: list[Transaction]) -> str:
    ...
    if len(transactions) == 0:
        logger.warning("search_individual_transfers получил пустые данные о транзакциях")
        return ""
    ...
    try:
        ...
        logger.error(f"search_individual_transfers был выполнен с ошибкой: {e}.")
        return ""

    if len(filtered_transactions) == 0:
        logger.warning("search_individual_transfers получил пустые данные о транзакциях после фильтрации")
        return ""
